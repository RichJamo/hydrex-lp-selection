...
function renderAll() {{
  ...
  for (const pair of PAIRS) {{
    ...
    Plotly.newPlot(divId, traces, layout, {{
      ...
      // Sync x-axis range across all pool charts. Use a SHARED global flag so that
      // programmatic relayouts on sibling charts don't recursively re-trigger sync.
      gd.on('plotly_relayout', ev => {{
        ...
        Promise.all(others.map(id => Plotly.relayout(id, update)))
          .finally(() => {{ window.__rangeSyncing = false; }});
      }});
    }});
  }}
}}
