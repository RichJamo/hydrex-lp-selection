...
function renderAll() {{
  ...
  for (const pair of PAIRS) {{
    ...
    // Most recent param config for header
    ...
    const traces = [{{
      ...
      line: {{ color, width: 2 }},
      name: pair,
      hovertemplate: (info.prefix || '') + '%{{y:' + info.tickFmt + '}}' + (info.suffix || '') + '<extra></extra>',
    }}];
    ...
    const layout = {{
      ...
      yaxis: {{
        type: yscale,
        title: {{ text: info.label, font: {{ size: 11, color: '#9aa4b2' }} }},
        gridcolor: 'rgba(255,255,255,0.07)',
        color: '#9aa4b2',
        tickprefix: info.prefix || '',
        ticksuffix: info.suffix || '',
        tickformat: info.tickFmt,
      }},
      shapes,
      annotations,
      hovermode: 'x unified',
...
document.querySelectorAll('input[name="metric"], input[name="yscale"], #show-params').forEach(el =>
